...
<div class="book-detail">
    ...
    <div class="detail-grid">
        ...
        <div class="detail-copy detail-reveal" style="--reveal-delay:90ms"><p class="eyebrow">{{ $book->categoryNames() }} · {{ $book->published_year ?? '—' }}</p><h1>{{ $book->title }}</h1><p class="detail-author">by <strong>@forelse($book->authors as $author)<a href="{{ route('authors.show', $author) }}" class="author-link">{{ $author->name }}</a>{{ !$loop->last ? ($loop->remaining === 1 ? ' & ' : ', ') : '' }}@empty Unknown author @endforelse</strong></p>
