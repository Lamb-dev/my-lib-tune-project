...
<div class="book-detail">
    ...
    <div class="detail-grid">
        ...
        <div class="detail-copy detail-reveal" style="--reveal-delay:90ms"><p class="eyebrow">{{ $book->categoryNames() }} · {{ $book->published_year ?? '—' }}</p><h1>{{ $book->title }}</h1><p class="detail-author">by <strong>{{ $book->authorNames() }}</strong></p>
            ...
            <div class="detail-actions">@if($book->hasEpubFile())<a href="{{ route('books.read',$book) }}" class="button button-dark"><i class="fa-solid fa-book-open"></i> Read online</a>@elseif(!$book->reading_url)<span class="button button-muted">Reading unavailable</span>@endif
            ...
            @endauth</div>
            <div class="book-facts"><div><span>AUTHOR</span>{{ $book->authorNames() }}</div><div><span>YEAR</span>{{ $book->published_year ?? 'Unknown' }}</div><div><span>FORMAT</span>{{ $book->hasEpubFile() ? 'EPUB · Online' : ($book->reading_url ? 'External link' : 'Catalogue only') }}</div></div>
        </div>
    </div>
